...
export const SettingsView: React.FC<SettingsViewProps> = ({ events, onUpdateEvents }) => {
  ...
  // Sort events by id (evt_1, evt_2, ...) then filter by type
  const sortedEvents = [...events].sort((a, b) => {
    const numA = parseInt(a.id.replace('evt_', '')) || 999;
    const numB = parseInt(b.id.replace('evt_', '')) || 999;
    return numA - numB;
  });

  const individualEvents = sortedEvents.filter(e => e.type === 'INDIVIDUAL');
  const pairEvents = sortedEvents.filter(e => e.type === 'PAIR');
  const teamEvents = sortedEvents.filter(e => e.type === 'TEAM');
